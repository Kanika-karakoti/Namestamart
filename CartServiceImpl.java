...
public class CartServiceImpl implements CartService {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (availableQty < prodQty) {
                    ...
                    DemandBean demandBean = new DemandBean(userId, product.getProdId(),
                            prodQty - availableQty);
                    ...
                    if (flag)
                        status += "<br/>Later, We Will Mail You when " + product.getProdName()
                                + " will be available into the Stock!";
                } else {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (prodQuantity > 0) {
                    ...
                    if (k > 0) status = "Product Successfully removed from the Cart!";
                } else {
                    ps2 = con.prepareStatement(
                            "delete from usercart where username=? and prodid=?");
        ...
        try {
            ...
            if (rs.next()) {
                if (prodQty > 0) {
                    ...
                    ps2.setString(3, prodId);
                    int k = ps2.executeUpdate();
                    if (k > 0) status = "Product Successfully Updated to Cart!";
                } else {
                    ps2 = con.prepareStatement(
                            "delete from usercart where username=? and prodid=?");
                    ps2.setString(1, userId);
                    ps2.setString(2, prodId);
                    int k = ps2.executeUpdate();
                    if (k > 0) status = "Product Successfully Updated in Cart!";
                }
            } else {
    ...
    public int getProductCount(String userId, String prodId) {
        ...
        try {
            ps = con.prepareStatement(
                    "select sum(quantity) from usercart where username=? and prodid=?");
            ps.setString(1, userId);
            ps.setString(2, prodId);
            rs = ps.executeQuery();
    ...
    public int getCartItemCount(String userId, String itemId) {
        ...
        try {
            ps = con.prepareStatement(
                    "select quantity from usercart where username=? and prodid=?");
            ps.setString(1, userId);
